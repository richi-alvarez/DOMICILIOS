    ...
    <div className="max-w-2xl space-y-6">
      ...
      <div className="rounded-2xl border border-warm-200 bg-warm-50 p-5 flex gap-4">
        ...
        <div className="flex-1 text-sm">
          <p className="font-semibold text-night-700 mb-1">{t('settings.paymentsForm.stripeTitle')}</p>
          <p className="text-night-400 leading-relaxed">
            {t('settings.paymentsForm.stripePre')}
            <a href="https://stripe.com" target="_blank" rel="noopener noreferrer" className="text-primary-500 underline">
              stripe.com
            </a>
            {t('settings.paymentsForm.stripeMid')}
            <a href="https://dashboard.stripe.com/apikeys" target="_blank" rel="noopener noreferrer" className="inline-flex items-center gap-0.5 text-primary-500 underline">
              {t('settings.paymentsForm.stripeLink')} <ExternalLink className="h-3 w-3" />
            </a>
          </p>
        </div>
      </div>
      ...
      <form onSubmit={handleSubmit} className="rounded-2xl border border-warm-200 bg-white p-6 space-y-5">
        ...
        <div className="flex items-center justify-between">
          <div>
            <p className="font-semibold text-night-800">{t('settings.paymentsForm.enableTitle')}</p>
            <p className="text-sm text-night-400">{t('settings.paymentsForm.enableDesc')}</p>
          </div>
          <button
            type="button"
        ...
        <div>
          <label className="mb-1.5 block text-sm font-semibold text-night-700">
            {t('settings.paymentsForm.publicKey')} <span className="text-warm-400 font-normal">{t('settings.paymentsForm.publicKeyHint')}</span>
          </label>
          <input
            type="text"
            value={publicKey}
        ...
        <div>
          <label className="mb-1.5 block text-sm font-semibold text-night-700">
            {t('settings.paymentsForm.secretKey')} <span className="text-warm-400 font-normal">{t('settings.paymentsForm.secretKeyHint')}</span>
          </label>
        ...
        <div>
          <label className="mb-1.5 block text-sm font-semibold text-night-700">
            {t('settings.paymentsForm.webhook')} <span className="text-warm-400 font-normal">{t('settings.paymentsForm.webhookHint')}</span>
          </label>
          <p className="mb-2 text-xs text-night-400">
            {t('settings.paymentsForm.webhookPoint')}
        ...
        {success && (
          <div className="flex items-center gap-2 rounded-xl border border-lime-200 bg-lime-50 px-4 py-3 text-sm text-lime-700">
            ...
            {t('settings.paymentsForm.savedOk')}
          </div>
        )}
      ...
      {publicKey && (
        <div className={`rounded-xl px-4 py-3 text-sm font-medium ${publicKey.startsWith('pk_live') ? 'bg-lime-50 text-lime-700 border border-lime-200' : 'bg-amber-50 text-amber-700 border border-amber-200'}`}>
          {publicKey.startsWith('pk_live') ? t('settings.paymentsForm.modeLive') : t('settings.paymentsForm.modeTest')}
        </div>
      )}
    </div>
  )
